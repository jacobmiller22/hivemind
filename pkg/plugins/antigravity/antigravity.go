...
func (p *Plugin) HandleEvent(ctx context.Context, event string, stdin io.Reader, stdout io.Writer) (*daemon.HivemindEvent, error) {
	// Read stdin
	stdinData, err := io.ReadAll(stdin)
	if err != nil {
		p.respondToAntigravity(stdout, event)
	...
	if input.ConversationID != "" {
		...
		evt = &daemon.HivemindEvent{
			Type:      "event",
			EventID:   "evt_" + p.generateRandomHex(8),
...
func (p *Plugin) generateRandomHex(n int) string {
	bytes := make([]byte, n)
...
func (p *Plugin) respondToAntigravity(stdout io.Writer, event string) {
