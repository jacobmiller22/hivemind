...
func main() {
	...
	// if err != nil {
	// 	log.Fatalf("Fatal error occurred: %+v\n", err)
	// }

	// logOut, err := os.Open(cfg.Log.File)
	// if err != nil {
	// 	log.Fatalf("Fatal error occurred: %+v\n", err)
	// }
	// ctx := context.Background()
	...
	// l := clog.WithContext(
	// 	ctx,
	// 	slog.New(slog.NewTextHandler(logOut, omniconfig.)),
	// )

	// demoFlag := flag.Bool("demo", false, "Run in offline interactive demo mode with mock data")
	// udsPath := flag.String("uds", client.ResolvePath("~/.config/hivemind/hivemind.sock"), "Custom path to the Unix Domain Socket")
	...
	// switch subcommand {
	// case "daemon":
	// 	runDaemon(*udsPath, *enabledToolsFlag, *antigravityDirFlag, *sessionsDirFlag, *filePollFlag)
	// case "install-hooks":
	// 	runInstallHooks()
	...
	// 		event = flag.Arg(1)
	// 	}
	// 	runHook(*udsPath, event)
	// case "help":
	// 	printHelp()
	// case "":
	// 	runClient(*udsPath, *demoFlag, *restartFlag)
	// default:
...
// 	log.Printf("Installing hivemind telemetry hooks...\n")

// 	home, err := os.UserHomeDir()
// 	if err != nil {
// 		log.Printf("Error: Unable to find home directory: %v\n", err)
// 		os.Exit(1)
// 	}
...
// 		log.Printf("Error: Unable to create plugin directory %s: %v\n", pluginDir, err)
// 		os.Exit(1)
// 	}

// 	hookDest := filepath.Join(pluginDir, "hivemind_hooks.py")
// 	if err := os.WriteFile(hookDest, hivemind.HivemindHooksPy, 0644); err != nil {
// 		log.Printf("Error: Failed to write hook script to %s: %v\n", hookDest, err)
// 		os.Exit(1)
// 	}
// 	log.Printf("✔ Copied hivemind_hooks.py to %s\n", hookDest)
...
// 				log.Printf("✔ %s already configured with PYTHONPATH\n", profile)
// 				continue
// 			}
...
// 			if err != nil {
// 				log.Printf("⚠ Failed to open shell profile %s for writing: %v\n", profile, err)
// 				continue
// 			}
// 			_, _ = f.WriteString(exportCmd)
// 			_ = f.Close()
// 			log.Printf("✔ Appended PYTHONPATH configuration to %s\n", profile)
// 			updatedCount++
// 		}
// 	}
...
// 	if updatedCount > 0 {
// 		log.Printf("👉 Please restart your terminal or run: source ~/%s (e.g. source ~/.zshrc) to apply environmental changes.\n", profiles[0])
// 	}
// }
...
// 				log.Printf("Daemon not running. Auto-spawning background daemon...\n")
...
// 					log.Printf("Warning: Failed to auto-spawn daemon: %v\n", errStart)
...
// 					}
// 					if !connected {
// 						log.Printf("Warning: Daemon auto-spawned, but UNIX domain socket did not become ready after %v. Degrading to demo mode.\n", timeout)
// 						runDemoMode = true
// 					}
// 				}
// 			} else {
...
// 		log.Printf("Error running Bubble Tea program: %v\n", err)
// 		os.Exit(1)
// 	}
// }

// func runMockFileEmitter() {
// 	log.Printf("Extracting and running mock file emitter...\n")
...
// 		log.Printf("Error writing mock file emitter: %v\n", err)
// 		os.Exit(1)
// 	}
...
// 	udsPath := customUdsPath
// 	if udsPath == "" {
// 		udsPath = daemon.DefaultUdsPath
// 	}

// 	slog.Info("runHook", "udspath", customUdsPath, "event", event)
...
//   -antigravity-dir <path>          Custom path to search for Antigravity transcripts (default: ~/.gemini/antigravity-treecli/brain)
